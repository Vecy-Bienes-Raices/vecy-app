...
const AcademyTutorChat = ({ module }) => {
    ...
    const processGeminiResponse = async (userText, currentMessages) => {
        // Construct the module content string for Gemini to understand context, or general context if no module
        const isModuleActive = module && module.lessons;
        const moduleContentText = isModuleActive
            ? module.lessons.map((l, i) => `Lección ${i + 1}: ${l.title}\nContenido: ${l.content}`).join('\n\n')
            : 'El estudiante navega libremente por la Academia VECY. Prepáralo para su proceso de certificación Inmobiliaria.';

        const currentTopic = isModuleActive ? module.title : 'Preparación Integral y Evaluativa VECY';

        const EDDU_SYSTEM_PROMPT = `
            ═══════════════════════════════════════════════
            MAESTRO EDDU — AULA SOCRÁTICA DE VECY ACADEMIA
            ═══════════════════════════════════════════════
 
            🔹 IDENTIDAD Y MANDATO:
            Eres el **Maestro Eddu**, un tutor empático, brillante y riguroso.
            Estás dictando una clase interactiva enfocada exclusivamente en este módulo:
            **TÍTULO:** ${currentTopic}
            **CONTEXTO ACTUAL:**
            ${moduleContentText}

            🔹 MÉTODO SOCRÁTICO (CRÍTICO):
            1. **NO DES RESPUESTAS FÁCILES NI MASTICADAS.** Eres un maestro, no un diccionario.
            2. Si el alumno hace una pregunta o pide ayuda sobre el módulo, guíalo con contra-preguntas para que él mismo llegue a la conclusión ("Has tocado un punto clave, pero piénsalo así: si tú fueras el juez, ¿cómo valorarías ese correo?").
            3. Si el alumno dice que entendió o pide un caso práctico, génerale un **Juego de Rol (Role-play)** o un mini caso de estudio ("Imagina que un arrendatario te dice X. ¿Qué le respondes basado en lo que leímos de la Ley 527?").

            🔹 PERSONALIDAD CACHACA Y TONO DE VOZ: 
            - Eres un verdadero cachaco moderno instruyendo. Elegante y carismático.
            - Háblale al alumno de "tú" (tuteo), NUNCA de "usted".
            - Usa términos como: "¡Ala!", "Carachas", "Chirriado", "Pisco", "Veci / Vecy / Vecinito".

            🔹 CAPACIDADES TÉCNICAS REITERADAS:
            - **TIENES CAPACIDAD NATIVA DE ESCUCHAR AUDIO.** Si recibes un audio, procésalo y responde. No digas que eres de "solo texto".

            🔹 REGLA DE ORO DE LA ACADEMIA:
            Limítate a evaluar y enseñar estrictamente lo relacionado con la Academia VECY y el Blindaje Probatorio. Tu meta es que el alumno comprenda el concepto profundamente antes del examen final. Si el alumno quiere cambiar a un tema no relacionado con el Derecho Inmobiliario o el módulo en sí, recuérdale con elegancia enfocarse en la clase.
        `;

        track('academy_tutor_message_sent', {
            module_id: module?.id || 'GENERAL_ACADEMY_VIEW'
        });
            ...
            <div className="bg-gradient-to-r from-[#1c1500] via-[#1c1c1c] to-[#1c1500] p-4 border-b border-[#bf953f]/25 flex items-center justify-between z-10 shadow-lg">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[#d4af37] text-[10px] uppercase tracking-widest">Tutor Interactivo VECY</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex flex-col items-center justify-center text-center animate-fade-in w-full max-w-lg mx-auto h-full space-y-6">
                        ...
                        <div className="space-y-4 px-4 bg-[#111]/90 backdrop-blur-sm p-6 rounded-xl border border-[#333] shadow-lg flex-shrink-0 w-full relative z-10 -mt-8">
                            <h2 className="text-xl md:text-2xl font-serif font-bold text-white tracking-tight">
                                Aula Socrática <span className="text-[#d4af37]">VECY</span>
                            </h2>
                            <p className="text-sm md:text-base text-gray-300 font-light leading-relaxed">
                                ¡Pláceme saludarte mi vecy! Soy el <strong className="text-[#d4af37] font-bold">Maestro Eddu</strong>, tu tutor personal para certificar tu idoneidad. ¿Qué dudas tienes sobre esta lección?
                            </p>
                            <div className="pt-1 flex justify-center">
                                <span className="text-[10px] font-bold text-[#bf953f] tracking-[0.2em] uppercase px-4 py-1.5 rounded-full border border-[#bf953f]/30 bg-[#bf953f]/10">
                                    Enfocado en: {module ? module.title : 'Formación General'}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div className="w-full p-3 bg-gradient-to-t from-[#0a0a0a] to-[#111] border-t border-[#333] z-20 shrink-0">
                <div className="relative flex flex-col bg-black border border-[#333] rounded-xl focus-within:border-[#bf953f]/70 focus-within:shadow-[0_0_15px_rgba(212,175,55,0.1)] transition-all overflow-hidden">
                    ...
                    <textarea
                        ...
                        placeholder="Pregúntale al Maestro Eddu..."
...
export default AcademyTutorChat;
