...
import AbogadoMiniChat from '../chat/AbogadoMiniChat';

const AbogadoFloatingWidget = () => {
    const [isChatOpen, setIsChatOpen] = useState(false);
    const { user } = useAuth();
    const { hasPaidSupport } = usePayment();
    const navigate = useNavigate();

    const handleChatOpen = () => {
        if (!user) {
            navigate('/login');
        } else if (!hasPaidSupport) {
            navigate('/#planes');
        } else {
            setIsChatOpen(true);
        }
                ...
                ">
                    ...
                    <div className="flex flex-col h-full pt-4 lg:pt-6">
                        <div className="flex items-center gap-2 mb-4 px-2 lg:px-6 flex-shrink-0">
                            ...
                            <span className="text-[#bf953f] text-xs font-bold uppercase tracking-[0.2em]">Bufete Virtual VECY</span>
                        </div>
                    ...
                    <AnimatePresence>
                        <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 hidden md:block opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <div className="bg-black border border-[#333] px-4 py-2 rounded-sm whitespace-nowrap shadow-2xl relative">
                                <span className="text-xs font-bold text-[#bf953f] uppercase tracking-widest">¿Dudas en tus negocios?</span>
                                <div className="text-[10px] text-gray-500 mt-1">Consúltale al <span className="text-white">Abogado Eddu</span></div>
...
export default AbogadoFloatingWidget;
